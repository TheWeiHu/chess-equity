...
def build(
    ...
    if model_name == "maia2":
        equity_desc = (
            "real rating-conditioned numbers from Maia-2's value head (--model maia2)"
        )
    else:
        equity_desc = (
            "an ILLUSTRATIVE rating skew, not a trained model (--model demo); "
    ...
    return {
        "_comment": (
            ...
            f"over a rating grid. Generated by web/build_demo.py. The equity is {equity_desc}."
        ),
        "model": model_name,
        "cp_engine": cp_engine,
        "game": {
