...
def test_focus_director_bias_wins_ties_and_small_margins():
    """A biased board (additive bonus) holds focus through ties and sub-margin rivals
    that an unbiased director would have ignored. ``decay=1.0`` isolates the bias knob
    from the task-0257 recency window so this asserts a TRUE tie, not a decayed one."""
    # Bias board 1 only.
    d = FocusDirector(bias=lambda b: FOCUS_BIAS_BONUS if b == 1 else 0.0, decay=1.0)
    d.note(0, 0.5)  # adopt board 0 silently
    # Board 1 ties board 0's drama: unbiased this is no cut, but the bonus tips it over.
...
def test_focus_director_bias_yields_to_a_much_bigger_swing():
    ...
    d = FocusDirector(bias=lambda b: FOCUS_BIAS_BONUS if b == 0 else 0.0, decay=1.0)
...
def test_focus_director_bias_still_fades_when_the_favorite_goes_quiet():
    ...
    d = FocusDirector(bias=lambda b: FOCUS_BIAS_BONUS if b == 0 else 0.0, decay=0.5)
