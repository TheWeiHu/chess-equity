...
The `baseline` is Lichess's **exact** published rating-blind cp→Win% logistic (`50 + 50·(2/(1+exp(-0.00368208·cp))−1)`, lila `rawWinningChances`), so every delta below is measured against the real curve the thesis must beat — not a strawman.
