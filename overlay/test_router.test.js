...
// ---- auto-advance off a finished board (task 0189), additional coverage ------
...
check("(0189-e) auto-advance composes with autofollow, and the director won't steal back to a finished board", () => {
  // Hair-trigger director (challengePlies:1, dramaMargin:0) so a single erupting tick
  // cuts — the anti-flap stickiness (0203) is exercised elsewhere; here we want the
  // simplest steal so the auto-advance composition is the thing under test.
  const r = O.makeBoardRouter({ autofollow: true, lockPlies: 3, challengePlies: 1, dramaMargin: 0 });
  r.learn(BOARDS_EVENT); // board 0 followed
