...
def _run_gate_badge(args: argparse.Namespace) -> int:
    """Render a shields.io endpoint badge from a gate verdict.json (task 0141).

    The README's ``gate`` badge is a shields.io endpoint pointed at a committed
    ``*.badge.json``. This regenerates that file from the same ``verdict.json`` ``gate-check``
    asserts, so the badge's green/red state is *driven by* ``verdict.json['pass']`` rather
    than hand-edited. Writes ``<report>.badge.json`` beside the verdict by default (mirroring
    how ``validate``/``headline`` write the verdict beside the report); pass ``--out -`` to
    print to stdout. Reuses ``gate-check``'s exit codes: 3 if the verdict is unreadable or
    carries an unexpected schema.
    """
    from pathlib import Path

    from chess_equity.validate.harness import gate_badge_payload

    path = Path(args.verdict)
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except FileNotFoundError:
        print(f"gate-badge: no verdict.json at {path}", file=sys.stderr)
        return GATE_CHECK_ERROR
    except (OSError, json.JSONDecodeError) as exc:
        print(f"gate-badge: cannot read {path}: {exc}", file=sys.stderr)
        return GATE_CHECK_ERROR

    try:
        badge = gate_badge_payload(payload)
    except ValueError as exc:
        print(f"gate-badge: {exc} in {path}", file=sys.stderr)
        return GATE_CHECK_ERROR
    ...
    if args.out == "-":
        ...
        # Default: the badge sits parallel to the verdict — `<report>.verdict.json` ->
        # `<report>.badge.json` (not `<report>.verdict.badge.json`).
        name = path.name
        default = name[: -len(".verdict.json")] if name.endswith(".verdict.json") else path.stem
        out_path = Path(args.out) if args.out else path.with_name(default + ".badge.json")
        out_path.write_text(text, encoding="utf-8")
        print(f"wrote {out_path} ({badge['message']})")
    return GATE_CHECK_PASS
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    gb = sub.add_parser(
        "gate-badge",
        help="render a shields.io endpoint badge JSON from a gate verdict.json (task 0141): "
        "the README `gate` badge's green/red state is driven by verdict.json['pass'], "
        "regenerated by this command instead of hand-edited",
    )
    gb.add_argument(
        "verdict",
        nargs="?",
        default=GATE_CHECK_DEFAULT_VERDICT,
        help="path to the verdict.json written beside a `validate --out` / `headline` "
        f"report; defaults to {GATE_CHECK_DEFAULT_VERDICT}",
    )
    gb.add_argument(
        "--out",
        help="where to write the shields endpoint JSON; default is `<report>.badge.json` "
        "beside the verdict, or `-` for stdout",
    )
    ...
    if args.command == "gate-badge":
        return _run_gate_badge(args)
